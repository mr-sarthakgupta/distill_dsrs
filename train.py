...
parser.add_argument('--k_warmup', default=100, type=int, help="Number of epochs over which the general Gaussian "
...
def main():
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        log(logfilename, "{}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}".format(
            epoch, after - before,
...
def simple_train(loader: DataLoader, model: torch.nn.Module, criterion, optimizer: Optimizer, epoch: int, k: int, device: torch.device, writer=None, k_warmup=None, step_counter=None):
    """
        If step_counter is not None, the step_counter saves the real step and k stores the k limit.
        Otherwise, k stores the real k
    :param loader:
    ...
    for i, (inputs, targets) in enumerate(loader):
        ...
        if True in torch.isnan(logits):
            ...
            # tyjk
        # if 
        # ic(torch.max(targets), torch.min(targets))
        # ic(torch.max(logits), torch.min(logits))
        ...
        # acc1, acc5 = accuracy(logits, torch.argmax(targets, dim = 1), topk=(1, 5))
        # losses.update(loss_xent.item(), batch_size)
        # losses_reg.update(consistency.item(), batch_size)
        # confidence.update(avg_confidence.item(), batch_size)
        # top1.update(acc1.item(), batch_size)
